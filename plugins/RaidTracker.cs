...
/*
    Added tracking for flamethrowers
    Fixed SaveExplosionData.NullReferenceException
*/

namespace Oxide.Plugins
{
    [Info("Raid Tracker", "nivex", "1.2.1")]
    ...
    public class RaidTracker : RustPlugin
    {
        ...
        private static List<Dictionary<string, string>> dataExplosions = new List<Dictionary<string, string>>();
        ...
        public class TrackingDevice : MonoBehaviour
        {
            ...
            {
                if (!playerId.IsSteamId() || !targetId.IsSteamId())
                {
                    return false;
                }
            ...
            private void OnDestroy()
            {
                ins.NextTick(() =>
                {
                    ...
                    if (string.IsNullOrEmpty(entityHit) || HasNoOwner(entityOwner) || IsOnSameTeam(ulong.Parse(playerId), entityOwner))
                    {
                        Destroy(this);
                        return;
                    }
                    ...
                    if (playerId == entityOwner.ToString() && _hideSelfFeed)
                    {
                        Destroy(this);
                        return;
                    }

                    CreateFeed(position, entityOwner, playerName, playerId, playerPosition, weapon, entityHit);
        ...
        private void OnExplosiveThrown(BasePlayer player, BaseEntity entity)
        {
            ...
            if (EventManager != null && EventManager.Call<bool>("isPlaying", player))
                return;
                    ...
                    case "delbefore":
                        {
                            if (args.Length == 2)
                            {
                                ...
                                if (DateTime.TryParse(args[1], out deleteDate))
                                {
                                    ...
                                    Player.Message(player, msg("Invalid Date", player.UserIDString, _chatCommand, DateTime.UtcNow.ToString("yyyy-mm-dd HH:mm:ss")));
                            }
                            else
                                SendHelp(player);
                        }
        ...
        private void SendHelp(BasePlayer player)
        {
            ...
            Player.Message(player, string.Format("/{0} delafter date - {1}", _chatCommand, msg("Help After", player.UserIDString, _chatCommand, DateTime.UtcNow.Subtract(new TimeSpan(_daysBeforeDelete, 0, 0, 0)).ToString())));
            Player.Message(player, string.Format("/{0} delbefore date - {1}", _chatCommand, msg("Help Before", player.UserIDString, _chatCommand, DateTime.UtcNow.ToString())));
